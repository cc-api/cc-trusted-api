...
class TestCCTrustedVmSdkTdx(test_sdk.TestCCTrustedVmSdk):
    ...
    def check_imr(self, imr_index: int, alg_id: int, ccel_data: bytes):
    ...
    def test_get_measurement(self):
        ...
        if "ima_hash=sha384" in cmdline:
            ...
            if (ima_policy is None or
                ...
                ccel_data = self.get_ccel_data()
                self.check_imr(0, alg.alg_id, ccel_data)
                self.check_imr(1, alg.alg_id, ccel_data)
                self.check_imr(2, alg.alg_id, ccel_data)
                self.check_imr(3, alg.alg_id, ccel_data)
